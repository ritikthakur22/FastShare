...
import FastShareServer from "./server.js";
import FastShareWsServer from "./ws-server.js";
...
if (conf.signalingServer) {
    ...
    if (!isValidUrl || containsProtocol) {
        console.error("SIGNALING_SERVER must be a valid url without the protocol prefix.\n" +
            "Examples of valid values: `fastshare.net`, `fastshare.example.com:3000`, `example.com/fastshare`");
        process.exit(1);
    }
...
// Start a new FastShare instance when an uncaught exception occurs
if (conf.autoStart) {
    process.on(
        'uncaughtException',
        () => {
...
const pairDropServer = new FastShareServer(conf);

if (!conf.signalingServer) {
    // Start websocket server if SIGNALING_SERVER is not set
    new FastShareWsServer(pairDropServer.server, conf);
} else {
...
console.log('\nFastShare is running on port', conf.port);
