class PeersUI {
    ...
    async _onKeyDown(e) {
        ...
        // close About FastShare page on Escape
        if (e.key === "Escape") {
...
class Dialog {
    ...
    hide() {
        ...
        document.title = 'FastShare | Transfer Files Cross-Platform. No Setup, No Signup.';
...
class ClipboardHistoryDialog extends Dialog {

    constructor() {
        super('clipboard-history-dialog');

        this.$clipboardHistoryBtn = $('clipboard-history');
        if (this.$clipboardHistoryBtn) {
            this.$clipboardHistoryBtn.addEventListener('click', _ => {
                if (this.isShown()) {
                    this.hide();
                } else {
                    this.show();
    ...
    show() {
        super.show();
        const container = this.$el.querySelector('.clipboard-list');
        if (!container) return;
        
        let history = JSON.parse(localStorage.getItem('fastshare-clipboard-history') || '[]');
        if (history.length === 0) {
            container.innerHTML = '<div style="padding: 20px; text-align: center; color: #888;">Clipboard history is empty.</div>';
            return;
        }
        ...
        history.forEach((item, index) => {
            ...
            text.textContent = item.length > 50 ? item.substring(0, 50) + '...' : item;
    ...
    static addToHistory(text) {
        ...
        localStorage.setItem('fastshare-clipboard-history', JSON.stringify(history));
    }
}
    ...
    async _displayFiles(peerId, displayName, connectionHash, files, imagesOnly, totalSize, badgeClassName) {
        ...
        if (files.length > 1) {
            ...
            try {
                ...
                filenameDownload = `FastShare_files_${year+month+date}_${hours+minutes}.zip`;
            } catch (e) {
                console.error(e);
        ...
        document.title = files.length === 1
            ? `${ Localization.getTranslation("document-titles.file-received") } - FastShare`
            : `${ Localization.getTranslation("document-titles.file-received-plural", null, {count: files.length}) } - FastShare`;
...
class ReceiveRequestDialog extends ReceiveDialog {
    ...
    _showRequestDialog(request, peerId) {
        ...
        document.title =  `${transferRequestTitle} - FastShare`;
        changeFavicon("images/favicon-96x96-notification.png");
...
class SendTextDialog extends Dialog {
    ...
    _send() {
        ...
        ClipboardHistoryDialog.addToHistory(text);
        Events.fire('send-text', {
            to: this.correspondingPeerId,
            text: text
        });
...
class ReceiveTextDialog extends Dialog {
    ...
    _setDocumentTitleMessages() {
        document.title = this._receiveTextQueue.length <= 1
            ? `${ Localization.getTranslation("document-titles.message-received") } - FastShare`
            : `${ Localization.getTranslation("document-titles.message-received-plural", null, {count: this._receiveTextQueue.length + 1}) } - FastShare`;
    }
...
class WebShareTargetUI {
    ...
    async evaluateShareTarget(shareTargetType, title, text, url) {
        if (shareTargetType === "text") {
            ...
            let openRequest = window.indexedDB.open('fastshare_store')
