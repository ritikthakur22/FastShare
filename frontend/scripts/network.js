class ServerConnection {
    ...
    _onDisplayName(msg) {
        ...
        // Add peerId to localStorage to mark it for other FastShare tabs on the same browser
...
class Peer {
    ...
    async _onFileReceived(fileBlob) {
        ...
        // include for compatibility with 'Snapdrop & FastShare for Android' app
    ...
    _onFileTransferCompleted() {
        ...
        if (!this._filesQueue.length) {
            ...
            Events.fire('files-sent'); // used by 'Snapdrop & FastShare for Android' app
        }
        else {
            this._dequeueFile();
        }
...
class PeersManager {
    ...
    _onPeerLeft(message) {
        ...
        if (message.disconnect === true) {
            // if user actively disconnected from FastShare server, disconnect all peer to peer connections immediately
